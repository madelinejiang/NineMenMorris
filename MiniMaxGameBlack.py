...
def MiniMaxGameBlack(board,depth,is_max):
    #is_max indicates white's turn, choosing maximizing position
    #else choose minizimizing position
    L=[]
    positions_count=0
    out_val=0
    saved_b=""
    ...
    if depth==0:
        se=MorrisV.StaticEstimation_MEgame(board)
        if is_max:
            if se>out_val:
                out_val=se
        else:
    ...
    for b in L:
        o_out,l_out,b_out=MiniMaxGameBlack(b,depth-1, not is_max)
...
def MiniMaxGameBlack(board,depth,is_max):
    board_w=SwapBW(board)
    out_val,position_count,saved_b=MiniMaxGame(board_w,depth,is_max)
    board_b=SwapBW(saved_b)
...
estimate, positions, board_out=MiniMaxGameBlack(board1.lower(),depth,False)
...
MiniMaxGameBlack(board1,5,False)
